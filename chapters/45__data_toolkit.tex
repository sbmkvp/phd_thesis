...
\marginnote[1em]{\fontsize{7}{7}\textit{The processing involves parsing json datarecieved for a single day at each location and aggregating them as number of probes requests recieved in every five minute intervals.}}
...
We can rule out the necessity of the above tools since our dataset is neither big enough nor fast enough.
...
This is where we come across the possibility of using standard Unix tools along with Bash scripting language to create a processing pipeline.
In some cases, a data processing pipeline made using command line Unix tools have been demonstrated to be 230x faster than using big data toolkits \cite{adam2014commandline}.
