...
\marginnote[1em]{\fontsize{7}{7}\textit{The processing involves parsing JSON data recieved for a single day at each location and aggregating them as number of probes requests recieved in every five minute intervals.}}
  ...
  \begin{center}
    \begin{tabular}{p{4cm}p{3cm}p{3cm}}
      ...
      \midrule
        ...
        \addlinespace[0.2cm]
				Impute missing value using time series analysis & imputeTS & R-Script\\
        \addlinespace[0.2cm]
				Visualise the results into maps and charts & ggplot2 & R-Script\\
        \addlinespace[0.2cm]
...
Figure \ref{figure:toolkit:time:parallel} compares the processing times of the Unix toolkit with a parallelised implementation.
...
\begin{figure*}[t]
  ...
  \caption{Outline of a `Medium data toolkit' for dealing with the Wi-Fi probe requests data}
  \label{figure:toolkit:visualisation}
\end{figure*}
